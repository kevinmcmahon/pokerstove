...
int main(int argc, char** argv)
{
    try
    {
        ...
        for (const string& hand : hands)
        {
            ...
            if (!handDists.back().parse(hand))
            {
                cerr << "Invalid hand: " << hand << endl;
                return EXIT_FAILURE;
            }
            for (size_t i = 0; i < handDists.back().size(); ++i)
            {
                if (handDists.back()[i].size() > evaluator->handSize())
                {
                    cerr << "Invalid hand: " << hand << endl;
                    return EXIT_FAILURE;
                }
        ...
        if (boardCards.size() > evaluator->boardSize())
        {
            cerr << "Invalid board: " << board << endl;
            return EXIT_FAILURE;
        }
        ...
        vector<EquityResult> results =
            showdown.calculateEquity(handDists, boardCards, evaluator);
        ...
        if (total == 0.0)
        {
            cerr << "Invalid showdown: no disjoint hand/board combinations"
                 << endl;
            return EXIT_FAILURE;
        }
